...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { name, url, method, headers, body: requestBody, cron_expression, task_type } = body;

    // Validate required fields
    if (!name || !url || !cron_expression || !task_type) {
      return NextResponse.json(
        { error: 'Missing required fields: name, url, cron_expression, task_type' },
        { status: 400 }
      );
    }
    ...
    return new Promise<NextResponse>((resolve) => {
      db.run(
        `INSERT INTO tasks (name, url, method, headers, body, cron_expression, task_type, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          name,
          url,
